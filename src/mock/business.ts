...
const moduleSeeds: ModuleSeed[] = [
  { module: 'service-calls', prefix: 'CALL', titles: ['VIP 客户来电排队超时', '退款咨询需要主管监听', '售后热线情绪升高'], channel: '电话', metric: '平均等待 42 秒', aiSuggestion: '建议主管开启耳语辅助，并优先接入高价值客户。' },
  { module: 'service-live-chat', prefix: 'CHAT', titles: ['机器人未命中密码重置意图', '多会话需要人工接管', '客户催促处理进度'], channel: '在线客服', metric: '转人工率 18%', aiSuggestion: '建议接管高风险会话，并引用最新密码重置知识。' },
  { module: 'omnichannel-email', prefix: 'MAIL', titles: ['账单争议邮件待审批', '附件缺失需要补充', '退款邮件线程需合并'], channel: '邮件', metric: '首响剩余 26 分钟', aiSuggestion: '建议合并重复线程并使用合规模板回复。' },
  { module: 'omnichannel-sms', prefix: 'SMS', titles: ['验证码发送失败重试', '营销短信退订风险', '通知模板待审核'], channel: '短信', metric: '送达率 96.4%', aiSuggestion: '建议暂停低互动任务，避开夜间发送窗口。' },
  { module: 'omnichannel-inbox', prefix: 'INBOX', titles: ['跨渠道客户待合并', '重复咨询待去重', '高优消息待分派'], channel: '全渠道', metric: '待分派 128 条', aiSuggestion: '建议按客户和意图聚合后统一指派。' },
  { module: 'operations-agents', prefix: 'AGT', titles: ['坐席负载不均', '技能组覆盖不足', '新人需要辅导'], channel: '坐席运营', metric: '在线 86 人', aiSuggestion: '建议把退款技能补充到晚班坐席组。' },
  { module: 'operations-scheduling', prefix: 'SCH', titles: ['周五晚高峰缺口', '调班申请待审批', '技能排班冲突'], channel: '排班', metric: '容量缺口 12 人', aiSuggestion: '建议提前开放售后加班班次。' },
  { module: 'operations-performance', prefix: 'PERF', titles: ['处理时长异常升高', '满意度目标复盘', '团队排名待确认'], channel: '绩效', metric: '达成率 91%', aiSuggestion: '建议对高时长低风险场景做定向辅导。' },
  { module: 'operations-quality', prefix: 'QA', titles: ['退款话术疑似违规', '质检申诉待审核', '脚本偏离样本'], channel: '质检', metric: '风险样本 9 条', aiSuggestion: '建议复核高风险录音并更新质检规则。' },
  ...
  { module: 'analytics-operations', prefix: 'OPS', titles: ['邮件队列积压', '渠道成本波动', '效率指标异常'], channel: '运营分析', metric: '服务量 +4.2%', aiSuggestion: '建议钻取邮件渠道和账单分类。' },
  { module: 'analytics-bi', prefix: 'BI', titles: ['SLA 指标口径冲突', '订阅报表失败', '导出任务排队'], channel: 'BI', metric: '报表 42 张', aiSuggestion: '建议统一 SLA 指标口径后重新发布。' },
  { module: 'analytics-sla', prefix: 'SLA', titles: ['高价值客户即将超时', '升级规则待启用', '履约率下降'], channel: 'SLA', metric: '履约率 90.1%', aiSuggestion: '建议提前 30 分钟触发预警。' },
  { module: 'analytics-risk', prefix: 'RSK', titles: ['异常租户切换', '投诉升级风险', '流失风险升高'], channel: '风险', metric: '高危 11 条', aiSuggestion: '建议冻结敏感操作并派单给安全负责人。' },
  { module: 'analytics-monitoring', prefix: 'MON', titles: ['知识检索延迟升高', '队列任务积压', '模型调用失败'], channel: '监控', metric: 'P95 184ms', aiSuggestion: '建议扩容向量检索副本。' },
  { module: 'analytics-alerts', prefix: 'ALT', titles: ['重复 SLA 告警', '严重告警待认领', '通知规则过宽'], channel: '告警', metric: '未处理 23 条', aiSuggestion: '建议合并重复告警并按区域路由。' },
  { module: 'system-permissions', prefix: 'RBAC', titles: ['角色数据范围过宽', '敏感权限待审批', '菜单授权待同步'], channel: '权限', metric: '权限变更 16 次', aiSuggestion: '建议把高风险角色收窄到部门级。' },
  { module: 'system-audit', prefix: 'AUD', titles: ['夜间导出异常', 'AI 决策审计待查', '登录风险会话'], channel: '审计', metric: '敏感操作 8 次', aiSuggestion: '建议复核夜间导出链路。' },
  { module: 'system-management', prefix: 'SYS', titles: ['业务开关待发布', '参数变更需审批', '字典项冲突'], channel: '系统', metric: '配置变更 12 次', aiSuggestion: '建议对高风险配置启用双人审批。' },
  { module: 'system-open-platform', prefix: 'API', titles: ['Webhook 失败率升高', 'API 密钥待轮换', '应用配额接近上限'], channel: '开放平台', metric: '调用 12.8k', aiSuggestion: '建议轮换密钥并检查回调地址。' },
  { module: 'dashboard-workbench', prefix: 'DSH', titles: ['今日 SLA 风险聚合', '投诉类工单上升', 'AI 介入建议待确认'], channel: '工作台', metric: '今日服务量 12.8k', aiSuggestion: '建议优先处理投诉上升和技术类时长偏高。' },
  { module: 'analytics-cockpit', prefix: 'CKP', titles: ['大屏核心指标待刷新', '区域服务波动', '风险墙需要升级'], channel: '驾驶舱', metric: '实时刷新 3 秒', aiSuggestion: '建议把 SLA 压力和风险队列提升到首屏。' },
];
