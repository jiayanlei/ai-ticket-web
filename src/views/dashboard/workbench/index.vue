...
const AUTO_FULLSCREEN_STORAGE_KEY = 'dashboardFullScreenShown';
...
const isScreenFullscreen = ref(shouldEnterInitialFullscreen());
...
function shouldEnterInitialFullscreen() {
  if (typeof window === 'undefined') {
    return false;
  }

  try {
    if (window.sessionStorage.getItem(AUTO_FULLSCREEN_STORAGE_KEY)) {
      return false;
    }

    window.sessionStorage.setItem(AUTO_FULLSCREEN_STORAGE_KEY, 'true');
  } catch {
    return window.innerWidth >= AUTO_FULLSCREEN_MIN_WIDTH;
  }
