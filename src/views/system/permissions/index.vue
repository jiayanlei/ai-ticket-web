<template>
  <div class="biz-page biz-page--permission">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="permission-shell"><aside><h2>角色列表</h2><button v-for="item in records" :key="item.id" @click="active = item">{{ item.title }}</button></aside><main><h2>权限树</h2><div class="permission-tree"><label v-for="node in ['工作台','工单','客户','AI','系统','审计']" :key="node"><input type="checkbox" checked /> {{ node }}</label></div></main><aside><h2>数据范围</h2><p>当前角色：{{ active?.title }}</p><a-radio-group value="dept"><a-radio value="self">本人</a-radio><a-radio value="dept">部门</a-radio><a-radio value="all">全部</a-radio></a-radio-group><h2>敏感权限</h2><p>{{ active?.aiSuggestion }}</p></aside></section>
...
const moduleName = 'system-permissions';
const pageTitle = '权限中心';
const pageDescription = '管理角色、菜单权限、动作权限、数据范围、权限分配和敏感权限预览。';
const primaryAction = '新增角色';
const metrics = [
  { label: '角色数量', value: '18', hint: '启用' },
  { label: '权限变更', value: '16', hint: '本周' },
  { label: '审批待办', value: '7', hint: '敏感权限' },
  { label: '越权风险', value: '4', hint: '需复核' },
];
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '权限中心已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
