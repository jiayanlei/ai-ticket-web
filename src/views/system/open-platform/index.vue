<template>
  <div class="biz-page biz-page--platform">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><div class="biz-topbar__controls"><div class="topbar-filter"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></div><a-space class="biz-topbar__actions"><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></div></header><section class="platform-shell"><main class="app-grid"><article v-for="item in records" :key="item.id"><strong>{{ item.title }}</strong><p>API Key: sk-***{{ item.code.slice(-4) }}</p><a-button @click="selectRecord(item)">查看回调</a-button></article></main><aside><h2>Webhook 日志</h2><p v-for="item in records" :key="item.id">{{ item.title }} / {{ item.status }}</p><a-button type="primary">轮换密钥</a-button></aside></section>
...
const moduleName = 'system-open-platform';
const pageTitle = '开放平台';
const pageDescription = '管理 API 应用、密钥、Webhook、回调日志、调用量、限流配额和集成设置。';
const primaryAction = '创建应用';
const statusOptions = [
  { label: '待处理', value: '待处理' },
  { label: '处理中', value: '处理中' },
  { label: '待审核', value: '待审核' },
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '开放平台已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
