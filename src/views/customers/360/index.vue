<template>
  <div class="biz-page biz-page--customer">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><div class="biz-topbar__controls"><div class="topbar-filter"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></div><a-space class="biz-topbar__actions"><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></div></header><section class="customer-shell"><aside><h2>客户档案</h2><strong>{{ active?.customer ?? '选择客户' }}</strong><p>价值等级 A / 续约临近</p><span>风险标签</span><span>高价值</span></aside><main><h2>互动时间线</h2><article v-for="item in records" :key="item.id"><strong>{{ item.title }}</strong><p>{{ item.description }}</p></article></main><aside><h2>下一步建议</h2><p>{{ active?.aiSuggestion }}</p><a-button type="primary" @click="changeStatus(active,'处理中')">安排回访</a-button></aside></section>
