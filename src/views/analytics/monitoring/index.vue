<template>
  <div class="biz-page biz-page--monitor">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="monitor-shell"><main class="health-grid"><article v-for="item in records" :key="item.id"><strong>{{ item.metric }}</strong><span>{{ item.title }}</span><div class="pulse"></div></article></main><aside><h2>运行日志</h2><p v-for="item in records" :key="item.id">{{ item.updateTime }} {{ item.title }}</p></aside></section>
...
const moduleName = 'analytics-monitoring';
const pageTitle = '系统监控';
const pageDescription = '监控服务健康、API 延迟、错误率、队列积压、AI 调用状态、告警订阅和运行日志。';
const primaryAction = '打开监控';
const metrics = [
  { label: '接口健康', value: '99.2%', hint: '近 1 小时' },
  { label: '模型延迟', value: '184ms', hint: 'P95' },
  { label: '任务积压', value: '316', hint: '队列' },
  { label: '容量水位', value: '72%', hint: '正常' },
];
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '系统监控已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
