<template>
  <div class="biz-page biz-page--monitor">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="monitor-shell"><main class="health-grid"><article v-for="item in records" :key="item.id"><strong>{{ item.title }}</strong><div class="pulse"></div></article></main><aside><h2>运行日志</h2><p v-for="item in records" :key="item.id">{{ item.updateTime }} {{ item.title }}</p></aside></section>
