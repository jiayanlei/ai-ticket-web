<template>
  <div class="biz-page biz-page--risk">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><div class="biz-topbar__controls"><div class="topbar-filter"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></div><a-space class="biz-topbar__actions"><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></div></header><section class="risk-shell"><main class="risk-lanes"><article v-for="level in ['严重','高','中','低']" :key="level"><h2>{{ level }}</h2><button v-for="item in records.filter(record => level === '严重' ? record.priority === '紧急' : true).slice(0, 3)" :key="item.id" @click="selectRecord(item)">{{ item.title }}</button></article></main><aside><h2>处置证据</h2><p>{{ active?.description }}</p><a-button type="primary" @click="changeStatus(active,'已完成')">关闭风险</a-button></aside></section>
...
const moduleName = 'analytics-risk';
const pageTitle = '风险预警';
const pageDescription = '管理风险事件、流失风险、投诉升级、情绪异常、风险等级、闭环处置和证据。';
const primaryAction = '处置风险';
const statusOptions = [
  { label: '待处理', value: '待处理' },
  { label: '处理中', value: '处理中' },
  { label: '待审核', value: '待审核' },
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '风险预警已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
