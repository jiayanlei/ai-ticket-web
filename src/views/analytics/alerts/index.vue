<template>
  <div class="biz-page biz-page--alert">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><div class="biz-topbar__controls"><div class="topbar-filter"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></div><a-space class="biz-topbar__actions"><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></div></header><section class="alert-shell"><aside><h2>严重级别</h2><span>严重</span><span>高</span><span>中</span><span>低</span></aside><main><h2>告警队列</h2><article v-for="item in records" :key="item.id"><a-tag color="red">{{ item.risk }}</a-tag><strong>{{ item.title }}</strong><p>{{ item.description }}</p><a-space><a-button type="primary" @click="changeStatus(item,'处理中')">认领</a-button><a-button @click="changeStatus(item,'已关闭')">关闭</a-button></a-space></article></main><aside><h2>降噪建议</h2><p>{{ active?.aiSuggestion }}</p></aside></section>
...
const moduleName = 'analytics-alerts';
const pageTitle = '告警中心';
const pageDescription = '管理告警列表、严重级别、认领、关闭、通知规则、关联对象、处理记录和降噪。';
const primaryAction = '认领告警';
const statusOptions = [
  { label: '待处理', value: '待处理' },
  { label: '处理中', value: '处理中' },
  { label: '待审核', value: '待审核' },
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '告警中心已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
