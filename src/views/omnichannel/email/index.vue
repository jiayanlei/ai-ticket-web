<template>
  <div class="biz-page biz-page--mail">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="filter-line"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></section><section class="mail-shell"><aside class="mail-list"><button v-for="item in records" :key="item.id" :class="{ active: active?.id === item.id }" @click="active = item"><strong>{{ item.title }}</strong><span>{{ item.customer }} / {{ item.status }}</span></button></aside><main class="mail-reader"><div class="mail-title"><span>邮件线程</span><h2>{{ active?.title ?? '选择邮件' }}</h2></div><p>{{ active?.description }}</p><div class="attachment-row"><span>附件 2</span><span>SLA {{ active?.metric }}</span><span>{{ active?.risk }}</span></div></main><aside class="draft-panel"><h2>AI 回复草稿</h2><p>{{ active?.aiSuggestion }}</p><a-space direction="vertical"><a-button type="primary" @click="changeStatus(active,'待审核')">提交审批</a-button><a-button @click="changeStatus(active,'已完成')">发送并关闭</a-button></a-space></aside></section>
...
const pageTitle = '邮件中心';
const pageDescription = '处理优先邮件、线程合并、附件校验、AI 摘要、草稿审批和转工单闭环。';
const primaryAction = '起草回复';
const metrics = [
  { label: '未读线程', value: '74', hint: '高优 12' },
  { label: '首响风险', value: '8', hint: '30 分钟内' },
  { label: 'AI 草稿', value: '21', hint: '待审批' },
  { label: '附件异常', value: '5', hint: '需补充' },
];
const statusOptions = [
  { label: '待处理', value: '待处理' },
  { label: '处理中', value: '处理中' },
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '邮件中心已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
