<template>
  <div class="biz-page biz-page--sms">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="filter-line"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></section><section class="sms-shell"><main><h2>发送任务</h2><div class="campaign-grid"><article v-for="item in records" :key="item.id"><span>{{ item.code }}</span><h3>{{ item.title }}</h3><p>{{ item.metric }}</p><a-progress :percent="item.priority === '紧急' ? 68 : 92" /></article></div></main><aside><h2>模板预览</h2><div class="phone-preview"><p>【AI Ticket】{{ active?.title ?? '服务通知' }}，请及时查看。</p></div><h2>失败重试</h2><button v-for="item in highRiskRecords" :key="item.id" @click="changeStatus(item,'处理中')">重试 {{ item.code }}</button></aside></section>
...
const moduleName = 'omnichannel-sms';
const pageTitle = '短信中心';
const pageDescription = '管理短信模板、发送任务、送达监控、失败重试、退订合规和效果分析。';
const primaryAction = '创建任务';
const metrics = [
  { label: '短信任务', value: '36', hint: '运行中' },
  { label: '送达率', value: '96.4%', hint: '较昨日 +1.2%' },
  { label: '失败重试', value: '118', hint: '待处理' },
  { label: '退订风险', value: '7', hint: '需复核' },
];
const statusOptions = [
  { label: '待处理', value: '待处理' },
  { label: '处理中', value: '处理中' },
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '短信中心已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
