<template>
  <div class="biz-page biz-page--prompt">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="prompt-shell"><aside><h2>模板库</h2><button v-for="item in records" :key="item.id" @click="active = item">{{ item.title }}</button></aside><main><h2>提示词编辑器</h2><a-textarea :value="'你是企业客服助手，请处理：' + (active?.title ?? '')" :rows="10" /><div class="variable-row"><span>{客户}</span><span>{工单}</span><span>{政策}</span></div></main><aside><h2>评测结果</h2><strong>88%</strong><p>{{ active?.aiSuggestion }}</p></aside></section>
