<template>
  <div class="biz-page biz-page--workflow">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="workflow-shell"><aside><h2>流程列表</h2><button v-for="item in records" :key="item.id" @click="active = item">{{ item.title }}</button></aside><main class="workflow-canvas"><article v-for="node in ['触发器','意图识别','知识检索','人工审批','发布/回滚']" :key="node"><span>{{ node }}</span></article></main><aside><h2>运行记录</h2><p>{{ active?.metric }}</p><p>{{ active?.aiSuggestion }}</p><a-button type="primary" @click="changeStatus(active,'已完成')">发布</a-button></aside></section>
...
const metrics = [
  { label: '运行流程', value: '38', hint: '启用' },
  { label: '失败节点', value: '7', hint: '待重试' },
  { label: '审批待办', value: '12', hint: '发布前' },
  { label: '实验命中', value: '18%', hint: '灰度' },
];
