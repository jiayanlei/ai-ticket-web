<template>
  <div class="biz-page biz-page--training">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="training-shell"><main class="course-grid"><article v-for="item in records" :key="item.id"><h3>{{ item.title }}</h3><p>{{ item.description }}</p><a-progress :percent="item.priority === '紧急' ? 38 : 78" /></article></main><aside><h2>能力雷达</h2><span v-for="skill in ['沟通','知识','流程','风险','效率']" :key="skill">{{ skill }}</span><p>{{ active?.aiSuggestion }}</p></aside></section>
...
const moduleName = 'operations-training';
const pageTitle = '培训中心';
const pageDescription = '管理课程、学习任务、知识测验、弱项推荐、进度跟踪、考试结果和能力画像。';
const primaryAction = '开始培训';
const metrics = [
  { label: '进行中课程', value: '18', hint: '本周' },
  { label: '通过率', value: '88%', hint: '考试' },
  { label: '技能缺口', value: '7', hint: '需补课' },
  { label: '模拟演练', value: '24', hint: 'AI 场景' },
];
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '培训中心已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
