<template>
  <div class="biz-page biz-page--quality">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="filter-line"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></section><section class="quality-shell"><aside><h2>质检样本</h2><button v-for="item in records" :key="item.id" @click="active = item">{{ item.title }}</button></aside><main><h2>证据时间线</h2><div class="evidence"><p>录音片段 00:42 - 01:16</p><p>脚本偏离：退款政策解释不完整</p><p>情绪：客户焦虑升高</p></div></main><aside><h2>评分</h2><strong class="score">86</strong><p>{{ active?.aiSuggestion }}</p><a-button type="primary" @click="changeStatus(active,'待审核')">提交复核</a-button></aside></section>
...
const moduleName = 'operations-quality';
const pageTitle = 'AI 质检';
const pageDescription = '检查会话样本、AI 评分、违规项、情绪和脚本分析、申诉复核和规则修正。';
const primaryAction = '运行质检';
const metrics = [
  { label: '质检样本', value: '320', hint: '今日' },
  { label: '合规风险', value: '9', hint: '高优' },
  { label: '脚本偏离', value: '27', hint: '待确认' },
  { label: '申诉待审', value: '6', hint: '质检主管' },
];
const statusOptions = [
  { label: '待处理', value: '待处理' },
  { label: '处理中', value: '处理中' },
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: 'AI 质检已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
