<template>
  <div class="biz-page biz-page--schedule">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="schedule-shell"><div class="demand-curve"><h2>峰值预测</h2><div v-for="n in 8" :key="n" :style="{ height: (32 + n * 8) + 'px' }"></div></div><main class="shift-calendar"><article v-for="day in ['周一','周二','周三','周四','周五','周六','周日']" :key="day"><h3>{{ day }}</h3><p>早班 24</p><p>晚班 18</p><span>缺口 {{ day === '周五' ? 12 : 2 }}</span></article></main><aside><h2>冲突处理</h2><button v-for="item in records" :key="item.id" @click="selectRecord(item)">{{ item.title }}</button></aside></section>
...
const moduleName = 'operations-scheduling';
const pageTitle = '排班调度';
const pageDescription = '规划班次、技能组排班、峰值预测、请假调班、冲突检测和 AI 排班建议。';
const primaryAction = '发布排班';
const metrics = [
  { label: '计划班次', value: '42', hint: '本周' },
  { label: '容量缺口', value: '12', hint: '周五高峰' },
  { label: '调班请求', value: '8', hint: '待审批' },
  { label: '冲突', value: '3', hint: '需处理' },
];
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '排班调度已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
