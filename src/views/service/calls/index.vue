<template>
  <div class="biz-page biz-page--call">
    <header class="biz-topbar"><p>{{ pageDescription }}</p><a-space><a-button @click="loadData">刷新</a-button><a-button type="primary" @click="openCreate">{{ primaryAction }}</a-button></a-space></header><section class="metric-strip"><article v-for="item in metrics" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small>{{ item.hint }}</small></article></section><section class="filter-line"><a-input v-model:value="query.keyword" allow-clear :placeholder="'搜索' + pageTitle + '、客户、负责人'" @press-enter="loadData" /><a-select v-model:value="query.status" allow-clear placeholder="状态" @change="loadData"><a-select-option v-for="item in statusOptions" :key="item.value" :value="item.value">{{ item.label }}</a-select-option></a-select><a-button type="primary" @click="loadData">查询</a-button></section><section class="call-shell"><aside class="queue-lanes"><h2>实时来电队列</h2><article v-for="(item,index) in records" :key="item.id" :class="{ active: active?.id === item.id }" @click="active = item"><span>队列 {{ index + 1 }}</span><strong>{{ item.title }}</strong><small>{{ item.customer }} / {{ item.metric }}</small></article></aside><main class="call-stage"><div class="call-control"><span>当前通话</span><h2>{{ active?.title ?? '暂无通话' }}</h2><p>{{ active?.description }}</p><a-space wrap><a-button type="primary" @click="changeStatus(active,'处理中')">监听</a-button><a-button>耳语</a-button><a-button danger>强插</a-button><a-button @click="changeStatus(active,'已关闭')">挂断</a-button></a-space></div><div class="ivr-map"><article v-for="node in ['IVR 接入','身份校验','意图识别','坐席接通','质检归档']" :key="node">{{ node }}</article></div></main><aside class="agent-wall"><h2>坐席状态墙</h2><span v-for="agent in ['陈沐阳 在线','郑宁 忙碌','刘薇 空闲','谢砚青 监听中']" :key="agent">{{ agent }}</span><h3>风险通话</h3><button v-for="item in highRiskRecords" :key="item.id" @click="selectRecord(item)">{{ item.title }}</button></aside></section>
...
const pageTitle = '呼叫中心';
const pageDescription = '运营实时呼叫队列、坐席状态、监听、耳语、强插、挂断、录音和质检闭环。';
const primaryAction = '监控队列';
const metrics = [
  { label: '排队来电', value: '42', hint: '最长等待 96 秒' },
  { label: '通话中', value: '86', hint: '坐席占用' },
  { label: '空闲坐席', value: '19', hint: '可接入' },
  { label: '风险通话', value: '6', hint: '需要主管' },
];
const statusOptions = [
  { label: '待处理', value: '待处理' },
  { label: '处理中', value: '处理中' },
...
const formState = reactive<BusinessRecordPayload>({
  ...
  aiSuggestion: '呼叫中心已生成 AI 建议，请优先处理高风险记录。',
  tags: [pageTitle],
});
const highRiskRecords = computed(() => records.value.filter((item) => item.risk.includes('高') || item.priority === '紧急'));
